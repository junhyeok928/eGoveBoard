...
function cuttingNameByLength(sName, nMaxLng) {
	...
	if (sName.length > nMaxLng) {
		...
		sTemp = sName.substring(0, nMaxLng) + "..."
...
function addImage(ofile) {
	// 파일 사이즈
	var ofile = ofile, sFileSize = 0, sFileName = "", sLiTag = "", bExceedLimitTotalSize = false, aFileList = [];
	...
	if (!!bExceedLimitTotalSize) {
		...
		// List 마크업 생성하기
		aFileList.push('	<li id="img' + nImageInfoCnt
				+ '" class="imgLi"><span>' + sFileName + '</span>');
		aFileList.push('	<em>' + sFileSize + '</em>');
		aFileList
				.push('	<a onclick="delImage(\'img'
						+ nImageInfoCnt
...
function callAjaxForHTML5(tempFile, sUploadURL) {
	var oAjax = jindo.$Ajax(sUploadURL, {
		...
		onload : function(res) { // 요청이 완료되면 실행될 콜백 함수
			...
			if (res.readyState() == 4) {
				...
				} else if (sResString.indexOf("LIMITFILESIZE_") > -1) {
					var maxSize = sResString.replace("LIMITFILESIZE_", "");
					maxSize = maxSize.trim();
					alert("첨부파일을 " + maxSize + " 이상 업로드 하실 수 없습니다.");
	...
	oAjax.header("Access-Control-Allow-Credentials", "true");
	oAjax.header("Access-Control-Allow-Origin", sUploadURL);
	oAjax.header("Access-Control-Max-Age", "86400");
	oAjax.header("Access-Control-Allow-Methods", "POST");
...
/**
 * 이미지 업로드 시작 확인 버튼 클릭하면 호출되는 msg
 */
...
function callFileUploader() {
	oFileUploader = new jindo.FileUploader(jindo.$("uploadInputBox"), {
		...
		// '/file_uploader_html5.jsp', //샘플 URL입니다.
		sUrl : '/user/cop/bbs/editorFileUpload',
		sCallback : location.href.replace(/\/[^\/]*$/, '') + '/callback.html', // 업로드
		// 이후에
		// iframe이
		// redirect될
		// 콜백페이지의
		...
		// "*.jpg", 구분자(;)
		sMsgNotAllowedExt : 'JPG, GIF, PNG, BMP 확장자만 가능합니다', // 허용할 파일의 형식이
		// 아닌경우에 띄워주는
		// 경고창의 문구
		bAutoUpload : false, // 파일이 선택됨과 동시에 자동으로 업로드를 수행할지 여부 (upload 메소드
		// 수행)
		bAutoReset : true
	// 업로드한 직후에 파일폼을 리셋 시킬지 여부 (reset 메소드 수행)
	}).attach({
		...
		error : function(oCustomEvent) {
			// 업로드가 실패했을 때 발생
			// oCustomEvent(이벤트 객체) = {
			// htResult : { (Object) 서버에서 전달해주는 결과 객체. 에러발생시 errstr 프로퍼티를 반드시
			// 포함하도록 서버 응답을 설정하여야한다.
			// errstr : (String) 에러메시지
			// }
			// }
 ...
 * 
 * @parameter aFileInfo [{},{},...]
 * @ex aFileInfo = [ { sFileName : "nmms_215646753.gif", sFileURL
 *     :"http://static.naver.net/www/u/2010/0611/nmms_215646753.gif", bNewLine :
 *     true }, { sFileName : "btn_sch_over.gif", sFileURL
 *     :"http://static1.naver.net/w9/btn_sch_over.gif", bNewLine : true } ]
 */
...
jindo.$Ajax.prototype.request = function(oData) {
	...
	if (req.addEventListener && !(navi.indexOf("Opera") > -1)
			&& !(navi.indexOf("MSIE") > -1)) {
		/*
		 * opera 10.60에서 XMLHttpRequest에 addEventListener기 추가되었지만 정상적으로 동작하지 않아
		 * opera는 무조건 dom1방식으로 지원함. IE9에서도 opera와 같은 문제가 있음.
		 */
		if (this._loadFunc) {
		...
		if (typeof req.onload != "undefined") {
			...
			/*
			 * IE6에서는 onreadystatechange가 동기적으로 실행되어 timeout이벤트가 발생안됨. 그래서
			 * interval로 체크하여 timeout이벤트가 정상적으로 발생되도록 수정. 비동기 방식일때만
			 * 
			 */
