...
<script type="text/javascript">
	...
	function fn_egov_updt_article(form) {
			...
			//물음표 제거
/* 			$("#nttCn").val(
					fn_replaceAll($("#nttCn").val(), unescape("%uFEFF"), ""));
			$("#nttCn").val(
					fn_replaceAll($("#nttCn").val(), unescape("%u200B"), ""));
 */
			var content = document.getElementById("nttCn").value;
...
<body onLoad="fn_egov_init();">
	...
	<form:form commandName="articleVO"
		action="${pageContext.request.contextPath}/user/cop/bbs/updateBoard.do" method="post"
		onSubmit="fn_egov_updt_article(document.forms[0]); return false;" enctype="multipart/form-data"
	>
		<div class="wTableFrm">
			<h2>${pageTitle}
			...
			>
				...
				<tbody>
					...
					<tr>
						...
						<c:set var="title">
							<spring:message code="comCopBbs.articleVO.updt.sjBoldAt" />
						</c:set>
					...
					<tr>
						...
						<td class="nopd" colspan="3">
							<textarea id="nttCn" name="nttCn" class="textarea" cols="75" rows="25"
								style="width: 520px;"
							><c:out value="${articleVO.nttCn}" escapeXml="false" /></textarea>
					...
					<!-- 유효기간 설정  -->
					<c:set var="title">
						<spring:message code="comCopBbs.articleVO.updt.ntceDe" />
					</c:set>
					<tr>
						<th><label for="ntceBgnde">${title}</label></th>
						<td class="left" colspan="3">
							<form:input path="ntceBgnde" title="${title} ${inputTxt}" size="70" maxlength="70"
								style="width:70px;" readonly="true"
							/>
							&nbsp;~&nbsp;
							<form:input path="ntceEndde" title="${title} ${inputTxt}" size="70" maxlength="70"
								style="width:70px;" readonly="true"
							/>
							<div>
						...
						<tr>
							...
							<td class="nopd" colspan="3">
								<input name="file_1" id="egovComFileUploader" type="file"
									title="<spring:message code="comCopBbs.articleVO.updt.atchFile"/>" multiple
								/>
								<!-- 첨부파일 -->
			...
			<div class="btn">
				...
				/>
				<!-- 수정 -->
				<span class="btn_s"><a href="<c:url value='/user/cop/bbs/selectBoardList.do' />"
