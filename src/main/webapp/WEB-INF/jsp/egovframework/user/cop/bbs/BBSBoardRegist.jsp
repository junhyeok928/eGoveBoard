...
<script type="text/javascript">
	...
	<form:form commandName="articleVO"
		action="${pageContext.request.contextPath}/user/cop/bbs/insertBoard.do" method="POST"
		onSubmit="fn_egov_regist_article(document.forms[0]); return false;"
	...
	>
		<div class="wTableFrm">
			...
			>
				...
				<tbody>
					...
					<tr>
						...
						<c:set var="title">
							<spring:message code="comCopBbs.articleVO.regist.sjBoldAt" />
						</c:set>
						<th><label for="sjBoldAt">${title}</label></th>
						<td class="left">
							<form:checkbox path="sjBoldAt" value="Y" />
							<div>
								<form:errors path="sjBoldAt" cssClass="error" />
							</div>
						</td>
					</tr>
					<!-- 글 내용  -->
					...
					<!-- 유효기간 설정  -->
					<c:set var="title">
						<spring:message code="comCopBbs.articleVO.regist.ntceDe" />
					</c:set>
					<tr>
						...
						<tr>
							...
							<td class="nopd" colspan="3">
								<input name="file_1" id="egovComFileUploader" type="file"
									title="<spring:message code="comCopBbs.articleVO.regist.atchFile"/>" multiple
								/>
								<!-- 첨부파일 -->
			...
			<div class="btn">
				...
				<!-- 등록 -->
				<span class="btn_s"><a
					href="<c:url value='/user/cop/bbs/selectBoardList.do' />?bbsId=${boardMasterVO.bbsId}"
